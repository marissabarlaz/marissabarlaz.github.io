...
```
What we need to do is create a summary function and pass that into ggplot. 

```{r}
ubdb %>% group_by(v) %>% summarise(meanF1 = mean(f1)) %>% ggplot(aes(x=v, y = meanF1))+geom_bar(aes(color = v, fill = v), stat = "identity")
```
