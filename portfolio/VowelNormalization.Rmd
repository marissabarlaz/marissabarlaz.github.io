...
$ERB = 21.4 \times log_{10}(1+F \times 0.00437))$
...
$Mel = 2595 \times log_{10}(1 + frac{F}{700})$
...
$F_n\prime = log_e(F_n) - \mu(log_e(F_n))$
...
$<F1\prime, F2\prime> = <\frac{2\times F2}{min(\mu*F2_v) +max(\mu*F2_v)}, \frac{3\times F1}{2 \times min(\mu*F1_v) +max(\mu*F1_v)}>$
...
$F_n = F_n - (\mu_0 + \mu_1 + \mu_2 + \mu_3)$ where $\mu_n$ is the log mean for f0-F3.
...
Note that the *vowels* package is the only one that does this method, as far as I am aware. I tried really hard and could not get this method to work and give me meaningful results (I was getting F1 = 5000 Hz and F2 = 1000 Hz, so...). Note that Labov said when writing about this method that it took up to 350 speakers before the sliding mean would change, so it is unlikely we will be using this method in our current research.
