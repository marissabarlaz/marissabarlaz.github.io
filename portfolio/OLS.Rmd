...
library(ggeffects)
library(effects)
BrP = read.csv("/Users/goldrch2/Desktop/Teaching/LING 490/Course Data/BrP_positionNP_omnibus.csv")
...
#somehow the data  got read in with spaces - this is recoding those affected levels without spaces
levels(BrP$position)[2] = "subject"
levels(BrP$survey)= c("listNP", "singleNP")
levels(BrP$NP)[1] = "barepl"
str(BrP)
```
...
BrP %>% mutate(rating = ordered(rating, levels=rev(levels(rating)))) %>% ggplot( aes(x = position, fill = rating)) + geom_bar(position = "fill") + scale_fill_manual(values = cbPalette) + theme_minimal()
...
newdat= data.frame(position = c("subject", "object"))
